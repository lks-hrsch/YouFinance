...
impl LocalCSV {
    pub fn new() -> Self {
        // We'll determine the actual path later or pass it in.
    ...
    fn find_csv_files_recursive(&self, dir: &std::path::Path, account_id: &str, depth: usize, csv_files: &mut Vec<PathBuf>) {
        ...
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.filter_map(|e| e.ok()) {
                let path = entry.path();
                if path.is_dir() {
                    if path.file_name().map_or(false, |n| n == account_id) {
            ...
            for csv_path in csv_files {
                ...
                for nt in new_transactions {
                    booked_transactions.push(Transaction {
                        transaction_id: None,
                        debtor_name: nt.debitor_name,
                        creditor_name: nt.creditor_name,
                        debtor_account: nt.debitor_iban.map(|iban| DebtorAccount { iban }),
                        ...
                        transaction_amount: TransactionAmount {
                            ...
                            amount: nt.amount.to_string(),
                        },
                        booking_date: Some(nt.date.clone()),
