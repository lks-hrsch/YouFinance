...
impl BankingApi for GoCardless {
    ...
    async fn get_access_token(&self) -> Result<AccessToken, ApiError> {
        ...
        let res = client.post(&format!("{}{}", self.base_url.to_owned(), "token/new/"))
            .headers(headers)
            .json(&map)
            .send().await?;
    ...
    async fn get_banks_by_country(&self, country: &str) -> Result<Vec<Bank>, ApiError> {
        ...
        let res = client.get(&format!("{}institutions/?country={}", self.base_url.to_owned(), country))
            .headers(headers)
            .send().await?;
    ...
    async fn connect_bank(&self, redirect: &str, institution_id: &str) -> Result<BankConnection, ApiError> {
        ...
        let res = client.post(&format!("{}requisitions/", self.base_url.to_owned()))
            .headers(headers)
            .json(&map)
            .send().await?;
    ...
    async fn get_bank_accounts(&self, bank_connection_id: &str) -> Result<BankAccounts, ApiError> {
        ...
        let res = client.get(&format!("{}requisitions/{}", self.base_url.to_owned(), bank_connection_id))
            .headers(headers)
            .send().await?;
    ...
    async fn get_account_transactions(&self, account_id: &str) -> Result<BankTransactions, ApiError> {
        ...
        let res = client.get(&format!("{}accounts/{}/transactions/", self.base_url.to_owned(), account_id))
            .headers(headers)
            .send().await?;
